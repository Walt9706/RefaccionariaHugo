...
class validadorregistro {
    ...
    private function validar_nombre($conexion,$nombre) {
        ...
        if (strlen($nombre) < 6) {
            return "El nombre debe de ser mayor que 6 caracteres";
        }
        if (strlen($nombre) > 13) {
            return "el nombre no puede ocupar mas de 13 caracteres";
        }
    ...
    private function validar_email($conexion,$email) {
        ...
        if (RepositorioUsuario::email_existe($conexion,$email)) {
            return "Este email ya esta en uso, Por favor, pruebeotro email o <a href='#'>intente recuperar su contraseña</a>";
        }
        return "";
    }
    ...
    private function validar_clave2($clave1, $clave2) {
        if (!$this->variable_iniciada($clave1)) {
            return "Primero debes de llenar la contraseña";
        }
